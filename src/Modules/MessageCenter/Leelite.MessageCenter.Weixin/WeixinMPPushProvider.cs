...
{
    public class WeixinMPPushProvider : PushProviderBase
    {
        private readonly IUserOpenIDService _userOpenIDService;
        private WeixinMPOptions _config = new WeixinMPOptions();

        public WeixinMPPushProvider(IUserOpenIDService userOpenIDService)
        {
            _userOpenIDService = userOpenIDService;

            ProviderName = "WeixinMP";
            ...
            */
            ...
            register.RegisterMpAccount(senparcWeixinSetting, "公众号");// DPBMARK_END


            //注册多个公众号或小程序（可注册多个）                                        -- DPBMARK MiniProgram
            register.RegisterWxOpenAccount(senparcWeixinSetting, "小程序");// DPBMARK_END
        ...
        {
            SendTemplateMessageResult result;
            if (string.IsNullOrEmpty(_config.MiniProgramAppId))
            {
                result = TemplateApi.SendTemplateMessage(_config.AppId, _userOpenIDService.GetOpenID(_config.AppId, record.UserId), record.TemplateCode, record.Url, new object());
            }
            else
            {
                result = TemplateApi.SendTemplateMessage(_config.AppId, _userOpenIDService.GetOpenID(_config.AppId, record.UserId), record.TemplateCode, "", new TemplateModel_MiniProgram()
                {
                    appid = _config.MiniProgramAppId,
                    pagepath = record.Url
                });
            }
            ...
            {
                WriteLineString($"WeixinMP:{record.MessageId}:{result.msgid}:{result.errcode}");
                return false;
            }
        }
